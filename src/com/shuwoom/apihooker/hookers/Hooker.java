...
public abstract class Hooker {
	...
	private final long startTimestamp = Calendar.getInstance().getTime()
			.getTime();
...
//			Log.v(Config.DEBUG_TAG,
//					new StringBuilder("before:{'method':'").append(className)
//							.append(".").append(((Method) method).getName())
//							.append("'}").toString());
//		}
//
//		@Override
...
//				Object resources, InterceptEvent event) {
//			Log.v(Config.DEBUG_TAG,
//					new StringBuilder("after:{'method':'").append(className)
//							.append(".").append(((Method) method).getName())
//							.append("'}").toString());
//
//		}
...
//	};
	...
	protected void hookMethods(final HookerListener listener,
			final String className, final Map<String, Integer> methodsToHook) {
		...
		MS.hookClassLoad(className, new MS.ClassLoadHook() {
			...
			public void classLoaded(final Class<?> classToHook) {
				...
				try {
					...
					ApplicationConfig.setPackageName(new StringBuilder(clz.getPackage().getName())
							.append(".").append(clz.getSimpleName()).toString());
				...
				while (iterator.hasNext()) {
					...
					Log.v(Config.DEBUG_TAG, new StringBuilder("start to hook ")
					...
					MS.hookMethod(classToHook, method, new MS.MethodHook() {
						...
						public Object invoked(Object resources, Object... args)
							...
							InterceptEvent event = new InterceptEvent(
									hookerName, intrusiveLevel, System
											.identityHashCode(resources),
									"unknown", className, methodName);
							...
							if (args != null) {
								for (Object arg : args) {
									if (arg != null) {
										...
										event.addParameter(arg.getClass()
												.getName(), argValue.toString());
									} else {
							...
							if (ApplicationConfig.getPackageName() != null) {
								event.setPackageName(ApplicationConfig
										.getPackageName());
							}

							final Context appContext = ApplicationConfig
									.getContext();
							if (appContext != null) {
								doBindService(appContext);
								Log.v(Config.DEBUG_TAG,
										new StringBuilder("hook context from****************>")
												.append(appContext.getPackageName()).toString());
							}

							if (listener != null) {
								listener.before(className, method, resources,
										event);
							}
							...
							if (result != null) {
								event.setReturns(result.getClass().getName(),
										result.toString());
							} else {
							...
							if (listener != null) {
								listener.after(className, method, resources,
										event);
							}
